...
Question for youself: Why `(num_blocks, block_size, head_size)` instead of one giant `(num_blocks * block_size, head_size)` tensor?
...
We need now a block allocator class to define the operations for managing physical blocks from the the block pool. It will allow us to allocate arbitrary amounts of blocks from the pool and free them if needed.
...
def test_block_allocation_lifecycle(scheduler):
    ...
    # 4. Simulate 4 decode steps (slots 13, 14, 15, 16)
    for _ in range(4):
