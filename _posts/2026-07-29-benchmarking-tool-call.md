...
3. **Parse the response** — The benchmark extracts tool calls by reading the `tool_calls` array from the response. For single-tool tasks, only the first tool call matters. For multi-tool tasks, all calls in a turn are collected.
...
This benchmark is a foundation. What it doesn't cover yet:
- **Real tool calls** — testing against actual APIs, not mocks
- **Latency** — how long does it take to orchestrate a chain?
- **Cost** — how many tokens does a multi-turn chain consume?
- **Temperature sensitivity** — does performance collapse at higher temperatures?
- **Cross-model comparison** — this is the tool; we need results across models

I'll keep building it out. Because if you're going to claim your model is "agentic," you should be able to prove it with a benchmark that tests more than "can it call a weather API."
