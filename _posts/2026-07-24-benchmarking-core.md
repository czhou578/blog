---
layout: post
title: "Developing my own Benchmark Pt. 2 (The Core Runner File)"
date: 2026-07-24
---

This is Part 2 of my effort to develop a more comprehensive benchmark for measuring the performance of large language models. Today, we will be focusing on building the `core_runner.py` file, which will serve as the foundational orchestrator for our benchmarking framework.

This file will be responsible for coordinating and running all the benchmarking tasks through command line arguments.

The code can be found here: [https://github.com/czhou578/model-benchmarks/blob/main/core_runner.py](https://github.com/czhou578/model-benchmarks/blob/main/core_runner.py)

## Helper Functions

We define several helper functions to capture the environment information (both hardware and software context) before running the benchmarks.

We have a function called `collect_environment()` that runs the command `nvidia-smi` to get the GPU information and the command `nvcc --version` to get the CUDA version. It also uses the `subprocess` module to run the command `sys.executable -c "import torch; print(torch.__version__)"` to get the PyTorch version and the command `sys.executable -c "import vllm; print(vllm.__version__)"` to get the vLLM version. 

The goal is to make the environment information reproducible for future benchmarking runs.

Lastly, we have the `_get_server_model_config()` function that fetches the model configuration from the running vLLM server. This includes architecture, model name, and other relevant information. This will be used in future benchmarking runs like the `run_flops_analysis` function.
...
One important concept to know is the `energy_above_idle_wh` baseline that is calculated in the function `_window_summary()`. This is the energy consumption above the idle baseline. It is significant because we can then track the energy consumption of the model server as it performs the benchmark relative to the idle baseline.

There is a subtlety worth understanding: the idle baseline is not captured *during* the benchmark itself. Instead, the benchmark runner calls `start_idle()` and `record_idle()` to sample GPU metrics while the GPU is sitting idle — typically by waiting a few seconds before firing any requests. Those idle samples are then passed to `_window_summary()` for each benchmark window, which subtracts that baseline from the active workload readings. This gives you the incremental energy that the benchmark task actually consumed, rather than the total wall-plug energy (which includes the GPU just sitting there loaded with model weights). Without this subtraction, the "energy per token" metric would be inflated by the static baseline power draw that exists regardless of whether the benchmark was running.
